...
export class DaxpiFormatter extends DefaultFormatter {
	...
	public formatMessage(meta: LogMeta, message: string): string {
		const topic = this.colored.yellow(meta.extra!.topic !== undefined ? `(${meta.extra!.topic})` : `(Phase)`);
...
Logger.setFormatter(new DaxpiFormatter());